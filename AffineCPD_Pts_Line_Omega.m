function [Np_pts,Np_line,P1_pts,P1_line,E_pts,E_line]=AffineCPD_Pts_Line_Omega(  Xi2,Ti2,  Xi4,Ti4, sigma2, omega,u)


[N2, D2]=size(Xi2);         [N4, D4]=size(Xi4);             %u = 1; 
if isfield(omega, 'points')
    omiga1 = omega.points;
else
    omiga1 = 0.5;
end
if isfield(omega, 'lines')
    omiga2 = omega.lines;
else
    omiga2 = 0.5;
end
...
%% 2D point
    if isempty(Xi2)
        ...
        son2 = ((2*pi)^(D2/2)) * (sigma2^(D2/2)) * (omiga1./(1-omiga1)) / u^(D2/2);
...
%% 4D line
    if isempty(Xi4)
        ...
        son2 = ((2*pi)^(D4/2)) * (sigma2^(D4/2)) * (omiga2./(1-omiga2)) / u^(D4/2);
