function h = vl_plotframe_oursNoCircle(frames,color, varargin)
...
if length(frames)==4; 
    frames(1:4) = [frames(1), frames(2), frames(4), frames(3)-pi/2]; 
end
...
% just points?
if zero_dimensional
%   h = plot(frames(1,:),frames(2,:),color,lineprop{:}) ;
  return ;
end
...
%   if do_arrows
%     allxf_arrows = xc + [0 A(1,2)] ;
%     allyf_arrows = yc + [0 A(2,2)] ;
%       h = line([allxf_arrows], ...
%                [allyf_arrows], ...
%                'Color',color(2),'LineWidth',LineWidth, ...
%                lineprop{:}) ;
%   end
